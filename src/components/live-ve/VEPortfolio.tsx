...
const OrbitIcon = ({ tool, x, y, size, delay }: { tool: { name: string; image: string }; x: number; y: number; size: string; delay: number }) => (
  <m.div
    ...
    style={{ left: `${x}%`, top: `${y}%` }}
  >
    <div className={`${size} rounded-2xl overflow-hidden border border-white/10 shadow-lg shadow-black/30`} style={{ background: "hsl(160 8% 10%)" }}>
    ...
    <section className="py-20 md:py-28" style={{ background: "hsl(160 8% 8%)" }}>
      <div className="max-w-[1380px] mx-auto px-6 md:px-12">
        ...
        <FadeInSection className="text-center mb-20">
          ...
          <div className="relative w-full max-w-[600px] md:max-w-[700px] mx-auto aspect-square">
            {/* Outer orbit ring */}
            ...
            {innerTools.map((tool, i) => {
              ...
              return <OrbitIcon key={tool.name} tool={tool} x={x} y={y} size="w-14 h-14 md:w-[68px] md:h-[68px]" delay={i * 0.08} />;
            })}
            ...
            {outerTools.map((tool, i) => {
              ...
              return <OrbitIcon key={tool.name} tool={tool} x={x} y={y} size="w-14 h-14 md:w-[72px] md:h-[72px]" delay={i * 0.06} />;
            })}
          </div>
        </FadeInSection>
