...
import { getMasterclassData } from "@/data/masterclassPages";
import type { MasterclassPageData } from "@/data/masterclassPages";
...
// Lazy-loaded "other masterclasses" component
import { lazy, Suspense } from "react";
const OtherMasterclasses = lazy(() => import("@/components/OtherMasterclasses"));

interface MasterclassDetailProps {
  slug?: string;
  initialData?: MasterclassPageData;
}

const MasterclassDetail = ({ slug: slugProp, initialData }: MasterclassDetailProps = {}) => {
  const params = useParams<{ slug: string }>();
  const slug = slugProp ?? params.slug;
  const [data, setData] = useState<MasterclassPageData | undefined>(initialData);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    if (slug) {
      getMasterclassData(slug).then((d) => {
        setData(d);
        setLoading(false);
      });
    } else {
  ...
  const portfolioImages = data.portfolioDisplayImages ?? [];
  const showPortfolio = data.showPortfolio !== false && portfolioImages.length > 0;
  ...
  return (
    <MotionProvider>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative z-10 bg-background w-full max-w-5xl mx-auto px-6 py-12 md:py-16 text-center -mt-16">
          <FadeInSection>
            ...
            <h1 className="font-display text-4xl sm:text-5xl md:text-7xl lg:text-8xl text-foreground uppercase tracking-wide leading-[0.9]">
              {data.discipline}
              <span className="sr-only"> — {data.name} Masterclass</span>
            </h1>
          </FadeInSection>
      ...
      {/* Other Masterclasses (lazy-loaded) */}
      <Suspense fallback={null}>
        <OtherMasterclasses currentName={data.name} />
      </Suspense>
