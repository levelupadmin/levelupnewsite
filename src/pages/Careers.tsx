      ...
      <div className="min-h-screen bg-[#0A0A0A] text-white font-sans selection:bg-primary/30">
        ...
        <section className="py-16 md:py-24 overflow-hidden">
          {(() => {
            ...
            const totalSlides = teamCards.length;

            return (
              <div>
                <div ref={emblaRef} className="overflow-hidden">
                  <div className="flex items-end gap-3 md:gap-5 px-6">
                    {teamCards.map((card, i) => {
                      const distance = Math.min(
                        Math.abs(i - selectedIndex),
                        totalSlides - Math.abs(i - selectedIndex)
                      );
                      const isCenter = distance === 0;
                      const isNear = distance === 1;
                      const height = isCenter ? "h-[400px] md:h-[520px]" : isNear ? "h-[340px] md:h-[440px]" : "h-[280px] md:h-[360px]";
                      const width = isCenter ? "min-w-[220px] md:min-w-[340px]" : isNear ? "min-w-[180px] md:min-w-[260px]" : "min-w-[140px] md:min-w-[200px]";

                      return (
                        <div
                          key={card.name}
                          className={`relative flex-shrink-0 ${width} ${height} rounded-xl overflow-hidden shadow-lg transition-all duration-500 ${!isCenter && !isNear ? "opacity-50" : ""}`}
                        >
                  ...
                  {teamCards.map((_, i) => (
                    <button
                      ...
                      className={`h-1 rounded-full transition-all duration-500 ${
                        i === selectedIndex ? "w-8 bg-[#FF6500]" : "w-2 bg-white/20 hover:bg-white/40"
                      }`}
